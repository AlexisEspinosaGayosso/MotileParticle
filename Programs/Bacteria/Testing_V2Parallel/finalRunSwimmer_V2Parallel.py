...
FactorEdge=np.array([                     10.0,                    10.0 ,                    9.0,                      8.0 ,                      7.0 ,                      6.0,                      5.0,                      4.0,                       3.0,                      2.0],  dtype=np.float)
...
resultsDirHere='/scratch/pawsey0106/espinosa/Python/Results_'+SwimmerTypeHere+'_Re'+str(ReynoldsHere)    
...
if SwimmerTypeHere=='Tumble':
    ...
    RealRunDuration=1 #[s]
    reversePctHere=None
    RealSemiDiameters=[0.000002,0.000001,0.000001] #[m]
    ...
    reversePctHere=0.5
    RealSemiDiameters=[0.000002,0.000001,0.000001] #[m] #Using the same for comparison, but vibrio values are larger=[4micro,2micro]
...
#Converted values
SemiDiametersHere=(np.array(RealSemiDiameters)/CollectorDiameter)/2 #This does not need the division by 2
...
print(SemiDiametersHere)
print(SwimSpeedHere)
print(RunDurationHere)
print(RotationalDiffusionHere)
print(SwimmerTypeHere)
print(reversePctHere)
...
Dc=1
YExtendedSeedLast=PassiveEdge[iRey]*FactorEdge[iRey]
YExtendedSeedIni=0.0
NPoints=48 #ideally 25 or more for production   
keepGoing=True
...
#The content of operations to perform
    cloudNameI=fieldHere.fieldType+'Re'+str(fieldHere.Reynolds)+'_'+particleHere+str(meanunround)+'_'+str(i)+'FinalRun'

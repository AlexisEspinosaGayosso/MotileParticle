...
completingModeText=sys.argv[3]
if completingModeText=='True':
    completingMode=True
    print('The case will be run in completing mode=True')
    print('That means, only the clouds without the FrozenPct.dat file will be reran')
elif completingModeText=='False':
    completingMode=False
    print('The case will be run in completing mode=False')
    print('That means, all the case will be run from scratch.')
else:
    print('The third argument is not True or False')
    print('That means this case will not be ran, as we need that information.')
    sys.exit()
...
resultsDirHere='/scratch/pawsey0106/espinosa/Python/Results_Extended100_'+SwimmerTypeHere+'_Re'+str(ReynoldsHere)
...
Dc=1
ExtendedFactor=100
YExtendedSeedLast=PassiveEdge[iRey]*ExtendedFactor
YExtendedSeedIni=PassiveEdge[iRey]*FactorEdge[iRey] #Starting from the Round2 Final_Run
...
#The content of operations to perform
    cloudNameI=fieldHere.fieldType+'Re'+str(fieldHere.Reynolds)+'_'+particleHere+'_'+SwimmerTypeHere+'_'+str(meanunround)+'_'+str(i)+'Extended'+str(ExtendedFactor)
    print('Checking For Y=',YExtendedSeed[i], 'cloudName=',  cloudNameI)
    cloudPreviouslyDone=False
    cloudDirHere=os.path.join(resultsDirHere, cloudNameI)
    if (not os.path.isdir(cloudDirHere)):
        os.makedirs(cloudDirHere)
    fileName=os.path.join(cloudDirHere,'FrozenPct.dat')
    if os.path.isfile(fileName):
        cloudPreviouslyDone=True
    if ((not cloudPreviouslyDone) or (not completingMode)):
        print('Solving for cloudName=',  cloudNameI)
...
####        print('Trying the ghost file')
####        fileGhost=os.path.join(cloudDirHere,'Ghost.dat')
####        with open(fileGhost, 'ab') as file_handle:
####            print('Writing the ghost file')
####            record=[0.1]
####            np.savetxt(file_handle,record)
    #End of the if ((not cloudPreviouslyDone) or (not completingMode))
